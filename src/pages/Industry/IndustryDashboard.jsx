...
import { Handshake, Check, ArrowRight, Sparkles, IndianRupee, Rocket, ExternalLink, Play, Layers, CheckCircle2, ShieldCheck, HeartHandshake } from 'lucide-react';
...
export default function IndustryDashboard() {
  ...
  const scalableReady = useMemo(() => challenges.filter((c) => (
    c.prototypeData?.isIndustryReady || STAGE_INDEX[c.status] >= STAGE_INDEX.prototype
  )), [challenges]);
  ...
  return (
    <DashboardLayout role="industry" nav={nav}
      ...
      <Routes>
        <Route index element={<Overview firm={firm} opportunities={opportunities} portfolio={portfolio} scalableCount={scalableReady.length} />} />
        <Route path="scalable-ready" element={<ScalableReadyProjects firm={firm} list={scalableReady} />} />
        ...
        <Route path="*" element={<Overview firm={firm} opportunities={opportunities} portfolio={portfolio} scalableCount={scalableReady.length} />} />
      </Routes>
    </DashboardLayout>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(120deg, #d97706 0%, #b45309 100%)' }}
      >
        <div className="relative flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-white text-[0.72rem] font-bold mb-2">
              <Rocket size={13} /> High-Readiness Solutions Pipeline
            </div>
            <h2 className="font-display text-2xl sm:text-3xl font-extrabold">
              Industry-Scalable Ready Projects
            </h2>
            <p className="text-white/85 text-sm mt-1 max-w-2xl">
              These working prototypes have been built and lab-tested by universities. Review their technical integration requirements and estimated budget to fund their commercial rollout and field deployment.
            </p>
          </div>
        </div>
      </div>
      ...
      {list.length === 0 ? (
        <Empty
          icon={Icons.Rocket}
          title="No ready prototypes available right now"
          sub="As soon as universities finish developing and testing working prototypes, they will appear here for industry adoption."
        />
      ) : (
        <div className="grid md:grid-cols-2 gap-5">
          {list.map((c) => {
            const proto = c.prototypeData || {
              title: `${c.category} Scalable Prototype`,
              abstract: c.proposal?.objective || c.description,
              trl: 6,
              estimatedFunding: 1500000,
              fundingRaised: c.partners.length ? 1500000 : 0,
              demoUrl: '#',
              videoDemoUrl: '#',
              integrationRequirements: [
                'Local power microgrid access',
                'Cellular IoT connectivity',
                'Gram Panchayat ground support',
              ],
              facultyLead: 'Prof. Innovation Lead',
              studentContributors: ['Research Scholars'],
              isIndustryReady: true,
            };

            const isAlreadyPartner = c.partners.some((p) => p.id === firm.id);
            const percentFunded = Math.min(
              100,
              Math.round(((proto.fundingRaised || 0) / (proto.estimatedFunding || 1)) * 100)
            );

            return (
              <div
                key={c.id}
                className="card p-5 space-y-4 border border-slate-100 flex flex-col justify-between hover:shadow-lg transition"
              >
                <div>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="chip bg-amber-50 text-amber-700 font-bold text-[0.7rem]">
                        TRL {proto.trl} Tested
                      </span>
                      <span className="chip bg-indigo-50 text-indigo-700 font-bold text-[0.7rem]">
                        {c.university?.short || 'University Team'}
                      </span>
                      {isAlreadyPartner && (
                        <span className="chip bg-emerald-50 text-emerald-700 font-bold text-[0.7rem]">
                          <CheckCircle2 size={11} className="mr-0.5" /> Funded by {firm.short}
                        </span>
                      )}
                    </div>
                  ...
                  <h3 className="font-display font-bold text-base text-slate-900 mt-2.5">
                    {proto.title}
                  </h3>
                  ...
                  {/* Financial Requirement Block */}
                  <div className="mt-4 p-4 rounded-xl bg-amber-50/60 border border-amber-200/80">
                    <div className="flex items-center justify-between text-xs font-bold text-amber-950 mb-1.5">
                      <span className="flex items-center gap-1.5">
                        <IndianRupee size={14} className="text-amber-700" /> Total Scaling Funding Needed:
                      </span>
                    ...
                    <div className="flex justify-between items-center text-[0.7rem] text-amber-900 mt-1.5 font-medium">
                      <span>Pledged: ₹{(proto.fundingRaised || 0).toLocaleString('en-IN')}</span>
                      <span className="font-bold">{percentFunded}% funded</span>
                    </div>
                  </div>

                  {/* Technical & Field Integration Prerequisites */}
                  <div className="mt-3.5">
                    <p className="text-[0.72rem] font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center gap-1.5">
                      <Layers size={13} className="text-indigo-500" /> Full-Scale Integration Requirements:
                    </p>
                    <div className="space-y-1.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                      {(proto.integrationRequirements || []).map((req, idx) => (
                        <div key={idx} className="flex items-start gap-2 text-[0.75rem] text-slate-700">
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
                  ...
                  <button
                    onClick={() => {
                      setSelectedProto({ ...c, proto });
                      setFundingAmount(String(proto.estimatedFunding || 1200000));
                    }}
      ...
      <Modal
        ...
        title={`Adopt & Pledge Funding for ${selectedProto?.code || 'Prototype'}`}
      >
        <form onSubmit={handlePledgeSubmit} className="space-y-4">
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100">
            <p className="text-xs text-slate-500">Project / Prototype</p>
            <p className="font-display font-bold text-sm text-slate-900 mt-0.5">
              {selectedProto?.proto?.title || selectedProto?.title}
            </p>
            <p className="text-[0.72rem] text-slate-500 mt-1">
              Developed by: <b>{selectedProto?.university?.name || 'Partner University'}</b>
            </p>
          </div>

          <div>
            <label className="label">Funding Commitment Amount (₹ INR) *</label>
            <div className="relative">
              ...
              <input
                ...
                className="field pl-8 font-bold text-slate-800"
              />
            </div>
            <p className="text-[0.7rem] text-slate-400 mt-1">
              Estimated total scaling budget: ₹{(selectedProto?.proto?.estimatedFunding || 0).toLocaleString('en-IN')}
            </p>
          </div>

          <div>
            <label className="label">Select Types of Industry Support Offered *</label>
            <div className="grid grid-cols-2 gap-2 mt-1">
              {SUPPORT_TYPES.map((type) => {
                const checked = supports.includes(type);
                return (
                  <button
                    key={type}
                    type="button"
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded border grid place-items-center ${
                        checked ? 'bg-amber-600 border-amber-600 text-white' : 'border-slate-300'
                      }`}
                    >
          ...
          <div>
            <label className="label">Partnership Note / CSR Alignment</label>
            <textarea
              rows={2}
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="e.g. Allocated under FY26 Clean Water CSR budget with engineering mentorship from our Jamshedpur plant."
          ...
          <div className="pt-3 border-t border-slate-100 flex justify-end gap-2">
            ...
            <button type="submit" className="btn btn-primary">
              <CheckCircle2 size={15} /> Confirm Funding & Partnership
            </button>
          </div>
        </form>
      </Modal>
