...
import { Bell, Menu, X, ChevronsLeft, Home, CheckCheck, LogOut } from 'lucide-react';
        ...
        <header className="sticky top-0 z-30 glass border-b border-white/60">
          <div className="flex items-center gap-3 px-4 sm:px-6 py-3">
            ...
            <div className="hidden sm:flex items-center gap-2.5 pl-3 border-l border-slate-200">
              <Avatar name={profile?.full_name ?? user?.name ?? r.label} size={34} />
              <div className="leading-tight min-w-0 max-w-[140px]">
                <p className="text-[0.8rem] font-bold text-slate-800 truncate">{profile?.full_name ?? user?.name ?? r.label}</p>
                <p className="text-[0.68rem] text-slate-400 truncate">{profile?.organization_name ?? user?.meta ?? r.label}</p>
              </div>
              <button
