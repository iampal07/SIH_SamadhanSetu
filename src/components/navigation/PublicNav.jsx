      ...
      <div className={cx('mx-auto max-w-6xl px-4 sm:px-6 transition-all duration-300')}>
        <div className={cx('flex items-center justify-between gap-4 rounded-2xl transition-all duration-300',
          ...
          <div className="hidden md:flex items-center gap-2 relative">
            ...
            {user ? (
              <Link to={userRolePath} className="btn btn-primary text-xs flex items-center gap-1.5">
                <UserCheck size={14} /> My Workspace
              </Link>
            ) : (
              <Link to="/login" className="btn btn-ghost text-xs flex items-center gap-1.5 text-indigo-600 font-bold hover:bg-indigo-50">
                <LogIn size={14} /> Sign In
              </Link>
            )}

            <button className="btn btn-ghost border border-slate-200" onClick={() => setMenu((m) => !m)}>
              <LayoutDashboard size={15} /> Portals <ChevronDown size={14} className={cx('transition', menu && 'rotate-180')} />
            </button>
            <AnimatePresence>
              {menu && (
                <>
